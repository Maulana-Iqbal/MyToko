...
if (!empty($export)) {
    ...
    header('Content-Disposition: attachment; filename=laporan_penerimaan_' . $tanggal . '_status_' . $persetujuan . '.xls');
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    @include('transaksi.invoiceHeader')
        <h3 style="text-align:center"><u>Laporan Penerimaan</u></h3>
        @if (!empty($tanggal))
        ...
        @if (!empty($persetujuan))
            <b>Status Penerimaan : </b> {{ $persetujuan }}<br>
        @endif
        ...
        <table id="datatable" width="100%" class="table-striped" border="1px" cellpadding="5" cellspacing="0">
            ...
            <tbody>
                ...
                $jumlah = 0;
                ?>
                @foreach ($penerimaan as $index => $p)
                    <tr>
                        <td align="center">{{ $index + 1 }}</td>
                        <td>{{ $p->nomor }}</td>
                        <td align="center">{{ tglIndo($p->tgl) }}</td>
                        <td>{{$p->akun->kode.' - '.$p->akun->name}}</td>
                        <td align="right">Rp.{{ number_format($p->jumlah, 0, ',', '.') }}</td>
