...
<div class="row">
    <div class="col-12">
        <div class="page-title-box">
            <h4 class="page-title">Ubah Transfer Barang</h4>
        </div>
        <form action="{{url('stock-transfer/update')}}" method="post" enctype="multipart/form-data">
            @csrf
...
<script>
    $(document).ready(function() {
        ...
        $('body').on('change', '#produk', function() {
            ...
            if (produk == id_produk && dari == id_dari) {
                swal("Pesan", 'Barang sudah ada di Daftar Transfer', "error");
                $("#resultSearch").html('')
                $("#canvasloading").hide();
                $("#loading").hide();
        ...
        $("#gudang").select2("trigger", "select", {
            data: {
                id: '{{$data->stock_jenis->where("jenis",2)->first()->gudang->id ?? ""}}',
                text: '{{$data->stock_jenis->where("jenis",2)->first()->gudang->nama ?? ""}}'
            }
        });
        $("#gudang2").select2("trigger", "select", {
            data: {
                id: '{{$data->stock_jenis->where("jenis",1)->first()->gudang->id ?? ""}}',
                text: '{{$data->stock_jenis->where("jenis",1)->first()->gudang->nama ?? ""}}'
            }
        });
